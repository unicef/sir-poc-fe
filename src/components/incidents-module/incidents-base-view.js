...
import {
  validateAllRequired,
  resetRequiredValidations
} from '../common/validations-helper.js';
import {
  makeRequest,
  handleBlobDataReceivedAndStartDownload
} from '../common/request-helper.js';

import { store } from '../../redux/store.js';
import { selectIncident } from '../../reducers/incidents.js';
import {
  fetchIncident,
  saveIncidentsAsDraft,
  changeOwnership
...
export class IncidentsBaseView extends connect(store)(PermissionsBase) {
  static get template() {
    ...
    return html`
      <style
        include="shared-styles form-fields-styles grid-layout-styles required-fields-styles data-table-styles"
      >
        :host {
      ...
      <div class="card">
        ...
        <div class="row-h flex-c buttons-area">
          <div>
            ${this.changeOwnership} ${this.saveBtnTmpl} ${this.goToEditBtnTmpl}
            ${this.submitIncidentTmpl} ${this.addImpactButtonTmpl}
          </div>
          <div>
            ${this.deleteDraftTmpl} ${this.resetButtonTmpl}
        ...
        <fieldset>
          ...
          <div>
            <div class="row-h flex-c p-relative">
              <div class="col col-4">
                ...
                >
                  ...
                  <span slot="message"
                    >[[selectedIncidentCategory.description]]<br />[[selectedIncidentCategory.comment]]
                  </span>
                </etools-info-tooltip>
              </div>
            ...
            <div class="row-h flex-c p-relative">
              ...
              <div class="col col-3">
                ...
                >
                  ...
                  <span slot="message"
                    >[[selectedThreatCategory.description]]</span
                  >
                </etools-info-tooltip>
              </div>
              <div class="col col-3">
            ...
            <template
              is="dom-if"
              if="[[isSpecialConditionSubcategory(selectedIncidentSubcategory)]]"
            >
              <div class="row-h flex-c" hidden$="[[useBasicLayout]]">
                <div class="alert-text">
                  ALERT: In an effort to protect the identity of victims, the
                  ONLY required fields for the
                  [[selectedIncidentSubcategory.name]] subcategory are Threat
                  Category, Incident Category, Incident Subcategory, Incident
                  Description, Region, Country, Incident Date, and Incident
                  Time. The victim should be informed that all other information
                  is VOLUNTARY.
                </div>
              </div>
            </template>
            ...
            <template
              is="dom-if"
              if="[[isTrafficAccident(selectedIncidentSubcategory, staticData)]]"
              restamp
            >
            ...
            <div class="row-h flex-c">
              <div class="col col-3 p-relative">
                ...
                >
                  ...
                  <span slot="message"
                    >[[selectedImpactSeverity.description]]</span
                  >
                </etools-info-tooltip>
              </div>
              <div
                class="col col-3"
                hidden$="[[isSafetyIncident(selectedIncidentCategory)]]"
              >
        ...
        <fieldset>
          <div hidden$="[[useBasicLayout]]">
            <review-fields
              data="[[incident]]"
              hidden$="[[hideReviewFields]]"
            ></review-fields>
          </div>
        </fieldset>

        <fieldset
          hidden$="[[hideRelatedDocsSection(readonly, state.app.offline,
        ...
        >
          ...
          <div
            class="margin-b"
            hidden$="[[hideUploadBtn(readonly, state.app.offline, incident.unsynced)]]"
          >
            <etools-upload-multi
          ...
          <div
            hidden$="[[!showAttachmentsList(offline, incident, incident.attachments,
          ...
          >
            ...
            <template is="dom-repeat" items="[[incident.attachments]]">
              ...
              >
                <div slot="row-data">
                  <span
                    class="col-data col-4 break-word"
                  ...
                  >
                    ...
                    <delete-attachment-button
                      hidden$="[[!canDeleteAttachment(incident.status, offline, readonly)]]"
                  ...
                  <span
                    class="col-data col-7"
                    title="[[item.note]]"
                    data-col-header-label="Note"
                  >
                    <paper-input
                      no-label-float
                      readonly$="[[readonly]]"
        ...
        <template is="dom-if" if="[[!readonly]]">
          <div class="row-h flex-c" hidden$="[[!state.app.offline]]">
            <warn-message
              hidden$="[[!_incidentHasTempIdOrNew(incidentId)]]"
          ...
          <div
            class="row-h flex-c"
            hidden$="[[!eventNotOk(incident.event, state.app.offline)]]"
          >
            <warn-message
              message="Can't save, selected event must be synced first"
            ></warn-message>
          </div>
        </template>
  ...
  _idChanged() {
    ...
    this.set(
      'incident',
      JSON.parse(JSON.stringify(selectIncident(this.state)))
    );
  ...
  selIncidentCategChanged(incidentCategory) {
    ...
    let selSubcategIsValid = incidentCategory.subcategories.find(
      s => s.id == this.incident.incident_subcategory
    );
  ...
  isTrafficAccident(incidentSubcategory) {
    ...
    let safetyCategory = this.staticData.incidentCategories.find(
      elem => elem.name === 'Safety'
    );
    let incident = safetyCategory.subcategories.find(
      elem => elem.id === incidentSubcategory.id
    );
  ...
  showSubType(crashType) {
    return this.staticData.crashSubTypes.filter(
      subType => crashType === subType.crash_type
    );
  }
  ...
  isSpecialConditionSubcategory(selectedIncidentSubcategory) {
    ...
    return (
      this.specialConditionSubcategories.indexOf(
        selectedIncidentSubcategory.name
      ) > -1
    );
  }
  ...
  isSubmitted(incident) {
    if (
      ...
      this.hasPermission('mark_incident_as_draft')
    ) {
      return true;
    }
  }
  ...
  canEdit(offline, status, unsynced) {
    return (
      (['created', 'rejected', 'submitted'].indexOf(status) > -1 &&
        this.hasPermission('change_incident') &&
        !offline) ||
  ...
  canViewBtn(offline, status, unsynced) {
    return (
      ['created', 'submitted'].indexOf(status) > -1 &&
      this.hasPermission('change_ownership_incident') &&
      !offline
    );
  }
  ...
  canDeleteAttachment() {
    return (
      this.incident &&
      this.hasPermission('delete_incidentattachment') &&
      !this.readonly &&
      !this.state.app.offline &&
      ['created', 'new'].indexOf(this.incident.status) > -1
    );
  }
  ...
  showAttachmentsList() {
    return (
      this.incident &&
      this.hasPermission('view_incidentattachment') &&
      !this.state.app.offline &&
      this.incident.attachments &&
      this.incident.attachments.length
    );
  }
